...
class EmbeddingComputer:
    ...
    def get_horizontal_split_patches(self, image, bbox, tag, idx, viz=False):
        ...
        split_boxes = [
            [x1, y1, x1 + w, y1 + h / 3],
            [x1, y1 + h / 4, x1 + w, y1 + (3 / 4) * h],
            [x1, y1 + (2 / 3) * h, x1 + w, y1 + h]
            # [x1, y1, x2, y2]
        ]
        split_boxes = np.array(split_boxes, dtype=np.int32)
        
        ## vis
        # if not self.debug and idx == 0:
        #     p = image[bbox[1] : bbox[3], bbox[0] : bbox[2]]
        #     cv2.imwrite("debug_tar_ori.jpg", p)
        #     p = image[split_boxes[1][1] : split_boxes[1][3], split_boxes[1][0] : split_boxes[1][2]]
        #     cv2.imwrite("debug_tar_split.jpg", p)
            
        # elif self.debug:
        #     p = image[bbox[1] : bbox[3], bbox[0] : bbox[2]]
        #     cv2.imwrite("debug_src_ori_%d.jpg"%(idx), p)
        #     p = image[split_boxes[1][1] : split_boxes[1][3], split_boxes[1][0] : split_boxes[1][2]]
        #     cv2.imwrite("debug_src_split_%d.jpg"%(idx), p)
        # self.debug = True
    ...
    def compute_embedding(self, img, bbox, tag):
        ...
        if not self.grid_off:
            embs = embs.reshape(bbox.shape[0], -1, embs.shape[-1])[:, 1]
        embs = embs.cpu().numpy()
    ...
    def initialize_model(self):
        if self.dataset == "mot17":
            ...
            #print("No dataset specified, using default model")
            path = "external/weights/mot20_sbs_S50.pth"
